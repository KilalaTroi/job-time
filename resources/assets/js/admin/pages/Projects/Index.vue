          ...
          <div class="col-sm-4">
            <div class="form-group">
              ...
              <input
                ...
                type="text"
                class="form-control"
                v-on:keyup="filterItems(search.keyword)"
  ...
  methods: {
    ...
    getDataProjects(projects) {
      if (projects.data.length) {
        let dataProjects = projects.data.map((item, index) => {
          let checkArchive =
            item.status === "archive"
              ? " <i style='color: #FF4A55;'>(Archived)</i>"
              : "";
          let checkTR = this.getObjectValue(
            this.types,
            item.type_id
          ).slug.includes("_tr")
            ? " (TR)"
    ...
    updateProject(item) {
      ...
      axios
        .patch(uri, item)
        .then(res => {
          let foundIndex = this.projects.data.findIndex(
            x => x.issue_id == item.issue_id
          );
