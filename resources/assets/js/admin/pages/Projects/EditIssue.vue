<template>
    <modal id="editIssue" :sizeClasses="modalLg" v-on:reset-validation="resetValidation">
        ...
        <div v-if="selectedItem">
            <div class="row">
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        <input v-model="selectedItem.i_name" type="text" name="issue" class="form-control">
                    </div>
                </div>
                <div class="col-sm-2">
                    <div class="form-group">
                        <label class="">{{$ml.with('VueJS').get('txtPage')}}</label>
                        <input v-model="selectedItem.page" type="number" name="page" class="form-control">
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="form-group">
                ...
                <div class="col-sm-2">
                    <div class="form-group">
                        ...
                        <input v-model="selectedItem.no_period" type="checkbox" name="no_period" @change="updatePeriod" class="form-control">
                    </div>
                </div>
            </div>
            <div class="row" v-if="has_period">
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <datepicker name="startDate" input-class="form-control" placeholder="Select Date" v-model="selectedItem.start_date" :format="customFormatter" :disabled-dates="disabledEndDates()" :language="getLangCode(this.$ml)">
                        </datepicker>
                    </div>
                </div> 
                <div class="col-sm-6">
                    <div class="form-group">
                        ...
                        <datepicker name="endDate" input-class="form-control" placeholder="Select Date" v-model="selectedItem.end_date" :format="customFormatter" :disabled-dates="disabledStartDates()" :language="getLangCode(this.$ml)">
                        </datepicker>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group text-right">
                <button @click="updateIssue(selectedItem)" type="button" class="btn btn-primary">
...
export default {
    ...
    methods: {
        ...
        currentPeriod(data) {
            if (this.dateFormat(data.start_date) === '--' && this.dateFormat(data.end_date) === '--') {
