    ...
    getDataSchedules(data) { 
      if (data.length) { 
        this.schedules = data.map((item, index) => {
          ...
          const codition = sDetail.length && this.selectTeam == 2 && type.slug != 'yuidea_image' && arrProjects.includes(item.p_id);
          ...
          if ( sDetail.length && this.selectTeam == 2 && arrProjects.includes(item.p_id) ) {
